...
def resample(data, target):
    ...
    resampled_df = pd.DataFrame(X, columns=data.drop(columns=TARGETS).columns)
...
if XGB:
    ...
    data_df_1 = resample(data_df, TARGETS[0])
    data_df_2 = resample(data_df, TARGETS[1])
    data_df_3 = resample(data_df, TARGETS[2])
    data_df_4 = resample(data_df, TARGETS[3])
    data_df_5 = resample(data_df, TARGETS[4])
    data_df_6 = resample(data_df, TARGETS[5])
    qmodel_1 = QModel(
        dataset=data_df_1, predictors=FEATURES, target_features=TARGETS[0]
    )
    qmodel_2 = QModel(
        dataset=data_df_2, predictors=FEATURES, target_features=TARGETS[1]
    )
    qmodel_3 = QModel(
        dataset=data_df_3, predictors=FEATURES, target_features=TARGETS[2]
    )
    qmodel_4 = QModel(
        dataset=data_df_4, predictors=FEATURES, target_features=TARGETS[3]
    )
    qmodel_5 = QModel(
        dataset=data_df_5, predictors=FEATURES, target_features=TARGETS[4]
    )
    qmodel_6 = QModel(
        dataset=data_df_6, predictors=FEATURES, target_features=TARGETS[5]
    )
