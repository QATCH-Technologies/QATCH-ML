...
try:
    ...
    class Constants():
        sf_min = 0.25 # 25.0 # 0.25 # 10.0
        sf_max = 25.0 # 69.0
        sf_step = 1.0
        smooth_factor_ratio = 0.75
        super_smooth_factor_ratio = sf_max # sf_min + random.random()*(sf_max - sf_min)
        baseline_smooth = 9 # must be an odd integer, greater than 1
        consider_points_above_pct = 0.60
        downsample_file_count = 20
        running_headless = True # increases debug logging verbosity
...
class ModelData():
    ...
    def normalize(self, arr, t_min = 0, t_max = 1):
        norm_arr = arr.copy() # [] # terribly inefficient code
        # return norm_arr
        try:
            diff = t_max - t_min
            diff_arr = max(arr) - min(arr) 
            if diff_arr == 0:
                diff_arr = 1 # prevent div by 0 error
            # for i in arr:
    ...
    def point_fit(self, point1, point2):
        x1, y1 = point1
        x2, y2 = point2
        m_slope = abs((y2 - y1) / (x2 - x1)) # force positive, we are interested in rate of change, independent of direction
    ...
    def find_intersect(self, line1, line2):
        m1, b1 = line1 # 1st element is slope, 2nd is offset
        m2, b2 = line2
        if m1 == m2: # avoid divide by zero warning
            Log.w("find_intersect(): Slopes are identical, no intersection possible")
            return (0,0) # indicate no intersection
        # Calculate intersection of these two fit lines
    ...
    def compare_slopes(self, fill_fit_slope, post_fit_slope, left_point_slope, right_point_slope, last_result = 0):
        ...
        # if fill_error < post_error:
        #     return 1 if last_result >= 0 else 0
        # if post_error < fill_error:
        #     return -1 if last_result <= 0 else 0
    ...
    def IdentifyPoints(self, data_path = None, times = None, freq = None, diss = None, start_at = None, sf = Constants.super_smooth_factor_ratio, i = -1, actual=[]):
        ...
        try:
            ...
            if times is None or freq is None or diss is None:
                with open(data_path, 'r') as f: # secure_open(data_path, 'r', "capture") as f:
                    csv_headers = next(f)
            ...
            # Calculate parameters for determining a 'good' vs 'bad' run
            is_good = True
            min_time = xs[0]
            ...
            #     else:
            #         Log.w(f"Missing XML file: Expected at \"{xml_path}\" for this run.")
            ...
            #     if False and QtWidgets.QMessageBox.No == QtWidgets.QMessageBox.question(None,
            ...
            if self._headless: 
                Log.d(TAG, f"SF Extend: {extend_smf}") # the nearest odd number of seconds (runtime)

            if extend_data:
                # check if data really is downsampled by checking timestamps around boundary
                ...
                if after > 5 * before: # '5' represents 1/4 of downsampling interval
            ...
            if extend_data:
                 # full resolution, to use for initial fill analysis
                xs_extend = xs.copy()
                ys_extend = ys.copy()
                # composite resolution, to use for smoothing
            ...
            # smooth_factor = num_datapoints / 10
            # smooth_factor = max(3, int(smooth_factor) + (int(smooth_factor + 1) % 2))
            # rought_fit = savgol_filter(ys, smooth_factor, 1, 1)

            num_datapoints = len(xs)
            smooth_points = 300
            dps = num_datapoints / total_runtime # data per sec
            smooth_factor = total_runtime * dps / smooth_points
            smooth_factor = max(3, int(smooth_factor) + (int(smooth_factor + 1) % 2))
            extend_smf = smooth_factor # not different, since we downsampled xs/ys
            ...
            # if extend_data:
            #     extend_len = len(ys[t_first_90_split:])
            #     ys_fit_ext = savgol_filter(ys[t_first_90_split:], min(extend_smf, extend_len), 1)
            #     ys_fit = np.concatenate((ys_fit[:t_first_90_split], ys_fit_ext))
            ...
            t_0p5 = next(x+0 for x,t in enumerate(xs) if t > 0.5) # 0       if xs[t_start] < 0.5 else next(x+0 for x,t in enumerate(xs) if t > 0.5)
            t_1p0 = next(x+1 for x,t in enumerate(xs) if t > 2.0) # t_start if xs[t_start] < 2.0 else next(x+1 for x,t in enumerate(xs) if t > 2.0)
            ...
            # points_with_maximal_crossings = [(0,[0])] # (step,zeros)
            # max_idx = np.where(super_smooth_diss_1st==100.0)[0][0]
            # sm_min_idx_1 = argrelextrema(super_smooth_diss_1st, np.less)[0]
            # min_idx = np.where(self.normalize(super_smooth_diss_2nd)==0.0)[0][0]
            # min_idx = next(y for y in sm_min_idx_1 if y > min_idx) 
            # for step in range(99, 0, -1):
            #     offset_smooth_diss_1st = super_smooth_diss_1st - step
            #     zeros = list(np.where(np.diff(np.sign(offset_smooth_diss_1st)))[0])
            #     if zeros[1] not in range(max_idx, min_idx):
            #         break
            #     if len(zeros) > len(points_with_maximal_crossings[0][1]): # and len(zeros) in range(7,10):
            #         if points_with_maximal_crossings[0][0] == 0:
            #             points_with_maximal_crossings = [(step,zeros)]
            #         else:
            #             points_with_maximal_crossings.append((step,zeros))
            #         # break
            #     elif len(zeros) == len(points_with_maximal_crossings[0][1]): # and len(zeros) in range(7,10):
            #         bail = False
            #         # for i,p in enumerate(points_with_maximal_crossings[-1][1]):
            #         #     if np.abs(zeros[i]-p) > 1000:
            #         #         bail = True
            #         #         break
            #         if bail: break
            #         points_with_maximal_crossings.append((step,zeros))
            # midpoint_idx = -1 # int(len(points_with_maximal_crossings) / 2)
            # optimal_step = points_with_maximal_crossings[midpoint_idx][0] #/2 - points_with_maximal_crossings[-1][0]/2
            # zone_boundaries = xs[points_with_maximal_crossings[midpoint_idx][1]] 
            # Log.d(f"zones = {zone_boundaries}")
            ...
            # expected_end = xs[min_val_2nd] + 10*initial_time_delta1
            # expected_end = xs[max_idx_1st] + 10*initial_time_delta2
            ...
            # resmooth everything that was smoothed before and will be used again
            ys_fit = savgol_filter(ys, extend_smf if extend_data else smooth_factor, 1)
            ys_freq_fit = savgol_filter(ys_freq, extend_smf if extend_data else smooth_factor, 1)
            ys_diff_fit = savgol_filter(ys_diff, extend_smf if extend_data else smooth_factor, 1)

            ys_diss_1st = savgol_filter(ys_fit, extend_smf if extend_data else smooth_factor, 1, 1) # int(Constants.sf_max), 1, 1) # TODO
            # if extend_data:
            #     extend_len = len(ys_fit[t_first_90_split:])
            #     ys_diss_diff_ext = savgol_filter(ys_fit[t_first_90_split:], min(extend_smf, extend_len), 1, 1)
            #     ys_diss_1st = np.concatenate((ys_diss_1st[:t_first_90_split], ys_diss_diff_ext))

            # operate tsmoothie on 1st deriv. dissipation data
            tsf = extend_smf if extend_data else smooth_factor # int(Constants.sf_max) # TODO 
            # tsf = fill_time * Constants.smooth_factor_ratio
            # tsf = int(tsf) + (int(tsf + 1) % 2)
            # if tsf < 3: tsf = 3
            ...
            # ys_diss_1st = savgol_filter(ys_diss_1st, smooth_factor, 1)

            # remove dc offset from 1st derivative, only showing peaks from zero baseline
            # deriv_smoother.smooth(ys_diss_1st)
            # super_poly_smoother = PolynomialSmoother(degree=4)
            # super_poly_smoother.smooth(ys_diss_1st[250:])
            # super_smooth_1st_deriv = super_poly_smoother.smooth_data[0]
            # super_smooth_1st_deriv = np.concatenate((ys_diss_1st[0:250], super_smooth_1st_deriv))

            ys_diss_2nd = savgol_filter(ys_diss_1st, extend_smf if extend_data else smooth_factor, 1, 1) # int(Constants.sf_max), 1, 1) # TODO
            # if extend_data:
            #     extend_len = len(ys_diss_1st[t_first_90_split:])
            #     ys_diss_2ndd_ext = savgol_filter(ys_diss_1st[t_first_90_split:], min(extend_smf, extend_len), 1, 1)
            #     ys_diss_2nd = np.concatenate((ys_diss_2nd[:t_first_90_split], ys_diss_2ndd_ext))

            deriv_smoother.smooth(ys_diss_2nd)
            ys_diss_2nd = deriv_smoother.smooth_data[0]
            ys_diss_2nd_abs = np.abs(ys_diss_2nd)
            # ys_diss_2nd = savgol_filter(ys_diss_2nd, 9, 1)

            ys_diss_diff_avg = np.average(ys_diss_1st) # AJR TODO 4/14: pick up here, this line is too high for the 109cp run
            ...
            # ys_diss_diff_avg = np.average(ys_diss_1st[zeros3[1]:])

            # ys_diss_1st = savgol_filter(ys_diss_1st, 21, 1)
            ...
            # ys_freq_fit = savgol_filter(ys_freq, extend_smf if extend_data else smooth_factor, 1)
            # if extend_data:
            #     extend_len = len(ys_freq[t_first_90_split:])
            #     ys_freq_fit_ext = savgol_filter(ys_freq[t_first_90_split:], min(extend_smf, extend_len), 1)
            #     ys_freq_fit = np.concatenate((ys_freq_fit[:t_first_90_split], ys_freq_fit_ext))
            ...
            # self.tbox_diff_factor.setText(f"{diff_factor:1.1f}")

            # if total_runtime > 90:
            #      # full resolution, to use for initial fill analysis
            #     ys_diff_extend = ys_diff.copy()
            #     # composite resolution, to use for smoothing
            #     ys_diff = np.concatenate((ys_diff[0:t_first_90_split:20], ys_diff[t_first_90_split:]))

            ys_diff_fit = savgol_filter(ys_diff, extend_smf if extend_data else smooth_factor, 1)
            # if extend_data:
            #     extend_len = len(ys_diff[t_first_90_split:])
            #     ys_diff_fit_ext = savgol_filter(ys_diff[t_first_90_split:], min(extend_smf, extend_len), 1)
            #     ys_diff_fit = np.concatenate((ys_diff_fit[:t_first_90_split], ys_diff_fit_ext))
            ...
            ys_diff_2nd = np.abs(deriv_smoother.smooth_data[0])
             ...
             # operate tsmoothie on raw dissipation data
            ...
            # create accumulated min/max of deriv. curves
            accumulated_1st = self.normalize(adjusted_diss_1st) + self.normalize(adjusted_freq_1st) + self.normalize(adjusted_diff_1st)
            # deriv_smoother.smooth(accumulated_1st)
            # baseline_1st = deriv_smoother.smooth_data[0]
            # baseline_1st = savgol_filter(accumulated_1st, 11, 1)
            # accumulated_1st -= baseline_1st
            ...
            # remove DC offset from 1st deriv
            if False:
                # super_smooth_1st_deriv = []
                for i in range(min(len(minima_idx_1), len(maxima_idx_1))):
                    maxima_val_1[i] -= minima_val_1[i]
                    # super_smooth_1st_deriv.append(maxima_val_1[i])
            ...
            # deriv_smoother.smooth(accumulated_2nd)
            # baseline_2nd = deriv_smoother.smooth_data[0]
            # baseline_2nd = savgol_filter(accumulated_2nd, 11, 1)
            # accumulated_2nd -= baseline_2nd
            ...
            level_smooth_factor = dps / 2 # fill_time * dps / 100
            ...
            if debug_plots:
                ...
                # plt.plot(xs, ys_freq_fit)
                # plt.plot(xs, super_smooth_freq_t1)
                # plt.plot(xs, ys_diff_fit)
                # plt.plot(xs, super_smooth_diff_t1)
            ...
            if debug_plots2:
                ...
                # plt.plot(xs, ys_freq_fit - super_smooth_freq_t1)
                # plt.plot(xs, np.abs(super_smooth_diff_t1 - ys_diff_fit))
                # plt.plot(xs, np.abs((ys_fit - super_smooth_diss_t1) + 
                #              (super_smooth_diff_t1 - ys_diff_fit)))

            # ### OLD APPROACH:
            # start_stop = sorted(maxima_sort_2[:2])
            # start_stop = [start_stop[0][0], start_stop[1][0]] #, start_stop[2][0], start_stop[3][0]]
            ...
            #     if len(poi_vals) == 0:
            #         Log.w(f"Warning: t_start was {t_start} out of {len(xs)} but that seems unlikely!")
            #         Log.w("Please confirm \"Begin Point\" during Step 1 point selection.")
            #     t_start = 100

            # smf = max(3, int(smooth_factor/10))
            # if smf % 2 == 0: smf += 1 # force odd number
            # ys_diff_fine = savgol_filter(ys_diff, smf, 1)
            # ys_diff_diff = savgol_filter(ys_diff, smf, 1, 1) #difference derivatives, not dissipation
            ...
            # ### NEW APPROACH 2024-01-25 ###
            # start_stop.clear()
            # start_stop = sorted(maxima_sort_1[:3])
            # start_stop = [start_stop[0][0], start_stop[-1][0]]
            # t_start = xs[start_stop[0]]
            # t_blip1 = 0
            # for x,y in minima_dict_1.items():
            #     t_blip1 = xs[x]
            #     if t_blip1 > t_start:
            #         break
            # t_delta = t_blip1 - t_start
            # t_quad1 = t_blip1 + 1 * t_delta
            # t_blip2 = t_blip1 + 2 * t_delta
            # t_quad2 = t_blip2 + 4 * t_delta
            # t_blip3 = t_blip2 + 4 * t_delta
            # t_quad3 = t_blip2 + 8 * t_delta

            # # # now fine tune each t_blip to find nearest 1st deriv. minima
            # # last_delta_2 = xs[-1] # max val
            # # last_delta_3 = xs[-1] # max val
            # # last_magna = maxima_sort_1[0] # max val
            # # adjusted_2 = False
            # # adjusted_3 = False
            # # for x,y in minima_dict_1.items():
            # #     if not adjusted_2:
            # #         this_delta_2 = t_blip2 - xs[x]
            # #         if xs[x] > t_blip2:
            # #             adjusted_2 = True
            # #             if y > last_magna:
            # #                 t_blip2 -= last_delta_2 # move to minima
            # #             else:
            # #                 t_blip2 -= this_delta_2
            # #         last_delta_2 = this_delta_2
            # #     if not adjusted_3:
            # #         this_delta_3 = t_blip3 - xs[x]
            # #         if xs[x] > t_blip3:
            # #             adjusted_3 = True
            # #             if y > last_magna:
            # #                 t_blip3 -= last_delta_3 # move to minima
            # #             else:
            # #                 t_blip3 -= this_delta_3
            # #         last_delta_3 = this_delta_3
            # #     last_magna = y
            ...
            #         if t_blip1 > t_start:
            #             found_b1 = True
            #     elif xs[x] > t_quad1 and not found_b2:
            #         t_blip2 = xs[x]
            #         found_b2 = True
            #     elif xs[x] > t_quad2 and not found_b3:
            #         last_diff_y = minima_sort_1[0][1]
            #         for i,y in enumerate(ys_diss_1st[x:]):
            #             if y <= last_diff_y:
            #                 t_blip3 = xs[x+i]
            #                 found_b3 = True
            #                 break
            ...
            ### NEW APPROACH 1/29/2024
            # if data_title[0:3] == "244":
            #     print("On the plot!")
            #     pass
            ...
            ys_diss_diff_avg = 0 # np.amax(accumulated_1st[t_0p5:t_1p0]) # np.average(np.abs(ys_diss_1st))
            possible_points1 = []
            this_max_point = (0,0)
            ...
            ys_diss_2ndd_avg = 0 # np.amax(accumulated_2nd[t_0p5:t_1p0]) # np.average(np.abs(ys_diss_2nd)) * 2
            possible_points2 = []
            this_max_point = (0,0)
            ...
            # mag1s = []
            # for p1 in possible_points1:
            #     mag1s.append(p1[1])
            # if len(mag1s) > 0:
            #     mag1s = self.normalize(mag1s)
            # mag2s = []
            # for p2 in possible_points2:
            #     mag2s.append(p2[1])
            # if len(mag2s) > 0:
            #     mag2s = self.normalize(mag2s)
            # sorted_possibilities = []
            # for i in range(len(possible_points1)):
            #     sorted_possibilities.append((possible_points1[i][0], mag1s[i]))
            # for i in range(len(possible_points2)):
            #     sorted_possibilities.append((possible_points2[i][0], mag2s[i]))
            sorted_possibilities = []
            # sorted_possibilities.extend(self.normalize_list_of_tuples(possible_points1))
            ...
            # seq_mags = []
            # for sp in sequential_possibilities:
            #     seq_mags.append(sp[1])
            # avg_seq_mag = np.average(seq_mags)
            if len(sequential_possibilities) > 0:
                # t_start = xs[sorted_possibilities[0][0]]
                # for p in sequential_possibilities:
                #     if p[1] > 2 * avg_seq_mag:
                #         t_start = xs[p[0]]
                #         break
                # for p in sequential_possibilities[::-1]:
                #     if p[1] > 2 * avg_seq_mag:
                #         t_blip3 = xs[p[0]] # xs[start_stop[-1][0]]
                #         break
                extend_dist = 0
                first_loop = True
                ...
                while True:
                    allow_break = False
                    # if t_blip3 - t_start > total_runtime / 10.0:
                    min_allowed_start = 3
                    max_allowed_start = avg_start + int((avg_stop - avg_start) / 10)
                    min_allowed_stop = avg_stop # avg_start + 2 * int((avg_stop - avg_start) / 3) # avg_stop # int((idx_end1 + avg_stop) / 2)
                    ...
                    if min_allowed_start < t_start < xs[max_allowed_start] and t_blip3 > xs[min_allowed_stop]:
                        ...
                        first_loop = False
                    extend_dist += 1
                    # check if length of sorted_possibilities is long enough
                    # otherwise, this can lead to an infinite loop if len() too short
                    if len(sorted_possibilities) < 4+extend_dist:
                        break # raise Exception("Too few posibilities, unable to Analyze.")
                    ...
                    if allow_break:
                        if start_stop[-1][0] < start_stop[-2][0]:
                            break
                    t_start = xs[avg_start] # xs[sorted(start_stop)[0][0]]
                    t_blip3 = xs[sorted(start_stop)[-1][0]]
            else:
                raise Exception("Too few possibilities to Analyze.")

            # TODO 2024-01-29: Lab day pick up here
            # TODO fine tune zone1 and zone4 by normalizing the points in those regions independently (if greater than 1 value exists in that zone)
            # for zone1, take the last time point with a normal magnitude greater than 0.25 (or, working from right-to-left, stop if |m| > 0.25)
            # for zone4, take the first time point with a normal magnitude greater than 0.25 (or, working from left-to-right, stop if |m| > 0.25)
            # once start/stop are precisely known, then calculate the delta and quadrant regions and move on to calculating zones 2 and 3 points
            # TODO Also, consider adding the "dead" region between zones 2 and 3 to zone 2 (some runs show the points exists in that "red" region)
            ...
            if t_quad0 < 2.5:
                t_quad0 = 2.5 # min allowed value if after initial settling period
                if t_quad1 < 3.0:
                    t_quad1 = 3.0
            # identify: start-of-fill, end-of-fill, post-point
            if extend_data:
            ...
            if diff_delta < 0:
                ...
                # invert data for start detection only
                norm_ys_diff = 1 - norm_ys_diff
                norm_ys_diff_fit = 1 - norm_ys_diff_fit
                diff_baseline = 1 - diff_baseline #np.average(norm_ys_diff_fit[t_0p5:idx_quad0])
            ...
            method = -1
            while True:
                # # break_diss = norm_ys_diss[this_idx-10] + up1[0][this_idx-10] - low1[0][this_idx-10]
                # # break_freq = norm_ys_freq[this_idx-10] + up2[0][this_idx-10] - low2[0][this_idx-10]
                # break_diff = norm_ys_diff[this_idx-10] + up3[0][this_idx-10] - low3[0][this_idx-10]
                # # if norm_ys_diss[this_idx] > break_diss and start_diss == start_idx:
                # #     start_diss = this_idx - 1
                # # if norm_ys_freq[this_idx] > break_freq and start_freq == start_idx:
                # #     start_freq = this_idx - 1
                # if norm_ys_diff[this_idx] > break_diff and start_diff == scan_start:
                #     start_diff = this_idx - 1
                #     break
                # if start_diss != scan_start and start_freq != scan_start and start_diff != scan_start:
                #     break
                # this_idx += 1
                ...
                if this_baseline < norm_ys_diff_fit[this_idx] < diff_midpoint:
                    if norm_ys_diff_fit[this_idx-1] > norm_ys_diff_fit[this_idx]:
                        # working right-to-left, we are within baseline, and slope is increasing
                        if start_diff == scan_start:
                            start_diff = this_idx
                            debug_point1 = start_diff
                            break # comment this out to scan the entire start region (required for training Plot #368)
                else:
                    start_diff = scan_start
            ...
            while True:
                ...
                # if norm_ys_freq[this_idx] > break_freq and start_freq == start_idx:
                #     start_freq = this_idx - 1
                if (norm_ys_diff[this_idx] > break_diff_high and start_diff < this_idx - 1): # or 
                    #norm_ys_diff[this_idx] < break_diff_low): # and start_diff == scan_start:
                    ...
                    else: # work left until you find a rel. minima
                        # steps_within_baseline = 0
                        while True:
                            # if break_diff_low < norm_ys_diff[this_idx] < break_diff_high:
                            #     steps_within_baseline += 1
                            # else:
                            #     steps_within_baseline = 0
                            # if steps_within_baseline < 2:
                            #     Log.d("NOTICE: Found start point within baseline region, no minima")
                            #     start_diff = this_idx
                            #     break
                            ...
                            if norm_ys_diff[this_idx - 1] > norm_ys_diff[this_idx]:
                                ...
                                break
                            # Secondary Method
                            if norm_ys_diff[this_idx] < up3_fill[0][debug_point1]: # (ys_diff_smooth[debug_point1] + break_diff_high)/2:
                ...
                if this_idx >= idx_quad1:
                    break
                this_idx += 1
            start_idx = start_diff # int(np.average([start_diss, start_freq, start_diff]))

            # if diff_delta < 0:
            #     # restore data after start detection only
            #     norm_ys_diff = 1 - norm_ys_diff
            #     norm_ys_diff_fit = 1 - norm_ys_diff_fit
            #     ys_diff_smooth = 1 - ys_diff_smooth
            #     up3[0] = temp_up3
            #     low3[0] = temp_low3
            ...
            while True:
                ...
                while True:
                    if num_attempts > 1 and max_pointfit[0] == 0:
                        pass # for breakpoint
                    this_idx += 1 # do first here
                    fit_start = idx_quad0 # TODO 2024-02-06: line changed, not run yet # start_idx if num_attempts == 1 else idx_quad0
                    # post_polyfit = np.polyfit(xs[fit_start:this_idx], norm_ys_diff_fit[fit_start:this_idx], 1)
                    # Calculate slope and offset of these two points
                    ...
                    # m_slope = (y2 - y1) / (x2 - x1)
                    # b_offset = y1 - m_slope * x1
                    # post_pointfit = (m_slope, b_offset)
                    post_pointfit = self.point_fit((x1,y1),(x2,y2))
                    # 1st element is slope, 2nd is offset
                ...
                p1 = (xs_extend[start_idx], norm_ys_diff_fit[start_idx])
                m1 = max_slope[0] # if max_slope[1] < min_slope[1] else min_slope[0]
                b1 = self.line_fit(p1, m1)
                max_pointfit = (m1, b1)

                # slopes = []
                # m1_idx = max_slope[1] if max_slope[1] < min_slope[1] else min_slope[1]
                # for idx in range(start_idx+1, m1_idx):
                #     p2 = (xs_extend[idx], norm_ys_diff_fit[idx])
                #     m, _ = self.point_fit(p1, p2)
                #     slopes.append(m)
                # if len(slopes) > 1 and sum(1 for slope in slopes if slope < 0) == 0:
                #     m1 = np.average(slopes)
                #     b1 = self.line_fit(p1, m1)
                #     max_pointfit = (m1, b1)

                p2 = (xs_extend[max_idx], norm_ys_diff_fit[max_idx])
                m2 = min_slope[0] # max_slope[0] if max_slope[1] > min_slope[1] else min_slope[0]
                ...
                # move end point right until we find the slopes are matching
                end_compare_result = 0
                this_idx = end_idx
                while True: #for this_idx in range(end_idx, idx_quad1):
                    # TODO test: variable window size, using raw data
                    num_fill_pts = end_idx - start_idx
                    window_size = 1 # max(1, int(num_fill_pts / 10))
                    # window_size = max(3, window_size + (window_size + 1) % 2)
                    # p3 = (xs_extend[max_idx+10],           norm_ys_diff_soft[max_idx+10])
                    ...
                    if end_compare_result == 0:
                        end_idx = this_idx
                        break
                    this_idx += end_compare_result # move left or right, depending on result of comparison
                    if this_idx <= start_idx or this_idx > idx_quad1:
                        break # bail if a limit is hit
                    # recalculate m1, as it will change as we track right
                    max_pointfit = self.point_fit(p1, p5)
                    m1 = max_pointfit[0]
                    # post_pointfit = self.point_fit(p6, p2)
                    m2 = 0 # post_pointfit[0]
                if end_compare_result != 0 and num_attempts < 2: # 2nd attempt may fail if there is no secondary fill to find
                    Log.w("Failed to find matching slope point! Using approximated end-of-fill point.")

                # stop_at_threshold = low3[0][end_idx]
                # for this_idx in range(end_idx, start_idx, -1):
                #     if ys_diff_smooth[this_idx] < stop_at_threshold:
                #         end_idx = this_idx + 1
                #         num_fill_pts = end_idx - start_idx
                #         break

                # if num_fill_pts > 50:
                #     end_idx -= int(num_fill_pts / 25)

                # this_idx = max_idx
                # while True:
                #     this_idx += 1 # do first here
                #     if norm_ys_diff[this_idx] < norm_ys_diff[this_idx-1] or this_idx >= idx_quad1: # TODO: use fit, or add hyster.
                #         if post_idx == scan_start:
                #             post_idx = this_idx
                #             break

                # post_compare_idx = post_idx + (max_idx - start_idx)
                # # if post_idx >= idx_quad1:
                # #     idx_quad1 += 10
                # try:
                #     post_pointfit = np.polyfit(xs[post_idx:post_compare_idx], norm_ys_diff_fit[post_idx:post_compare_idx], 1)
                # except:
                #     Log.e("ERROR: PolyFit() failed to find a linear fit, using single point delta fitting instead")
                #     p1 = (xs[post_idx], norm_ys_diff_fit[post_idx])
                #     p2 = (xs[post_idx+1], norm_ys_diff_fit[post_idx+1])
                #     post_pointfit = self.point_fit(p1, p2)
                # # m1, b1 = max_polyfit # 1st element is slope, 2nd is offset
                # # m2, b2 = post_pointfit
                # # # Calculate intersection of these two fit lines
                # # x_intersection = (b2 - b1) / (m1 - m2)
                # # y_intersection = m1 * x_intersection + b1
                ...
                # end_idx = next(x for x,y in enumerate(xs) if y >= x_intersection) - 1

                # # modify behavior of end and post for debugging
                # end_idx = max_idx
                # for i,p in sequential_possibilities: # TODO: This does not always find a point, what do then?
                #     if i < idx_quad1:
                #         if i <= max_idx: continue
                #         if p < 0.20: continue
                #     post_idx = i # point greater than exit, with >20% probability of being a spike
                #     break

                # check for break condition (end_idx intersects at or nead of zone1/quad1)
                # if post_idx == idx_quad1 or max_polyfit[0] < 1.0 or end_idx < last_end_idx:
                # if post_idx > idx_quad0/2 + idx_quad1/2 or num_attempts >= 2: # TODO: the 1st half of this logic is a lie
                if num_attempts >= 1:
                    if best_end_idx != 0: # not first attempt
                        ...
                        if (end_idx - start_idx < best_end_idx - best_start_idx or
                            ...
                            pass # ignore this fill if it is smaller in time OR magnitude than last one
                        else: # only keep it if fill region got bigger
                    ...
                    else: # or if we had nothing saved yet
                        best_start_idx = start_idx
                        best_end_idx = end_idx
                        best_post_idx = post_idx
                        best_num_fill_pts = num_fill_pts
                ...
                # move to try to find another set of points within this region, if found this is just inflow to be ignored
                scan_start = end_idx
                start_idx = scan_start
                end_idx = scan_start
                post_idx = scan_start
            ...
            # begin section to fine-tune final blip exit region points
            # identify: blip3
            ...
            if diff_delta < 0:
                # invert (again) after finding all points, for plotting
                # norm_ys_diff = 1 - norm_ys_diff
                # norm_ys_diff_fit = 1 - norm_ys_diff_fit
            ...
            # norm_freq_raw = self.normalize(ys_freq)
            ...
            # norm_diff_raw = self.normalize(ys_diff)
            ...
            if idx_max_freq != -1 and idx_max_diff != -1:
                ...
                # t_blip3 = xs[max(idx_max_freq, idx_max_diff)]
                if norm_freq_fit[-1] < idx_max_diff:
                    t_blip3 = xs[idx_max_freq]
                else:
                    t_blip3 = xs[idx_max_diff]
                # avg_stop = next(x for x,y in enumerate(xs) if y >= t_blip3)
            elif idx_max_freq != -1:
            ...
            # # check 2nd dissipation deriv. (only if the above FREQ/DIFF test fails)
            # # if the largest maxima in zone4 is above 60% then this IDX is the end
            # max_agree_delta = 3.0 # dig further if time delta is greater than this (secs)
            # diss_threshold = 0.1 # must be in range (0, 1); if 0: always trigger, if 1: never
            # if ((idx_max_freq == -1 and idx_max_diff == -1) or # neither FREQ nor DIFF found -or-
            #     (idx_max_freq != -1 and idx_max_diff != -1 and # both FREQ and DIFF found, and
            #      abs(xs[idx_max_freq] - xs[idx_max_diff]) > max_agree_delta)): # they disagree!
            #     idx_quad5 = next(x for x,y in enumerate(xs) if y >= t_quad5)
            #     try:
            #         idx_quad6 = next(x for x,y in enumerate(xs) if y >= t_quad6)
            #     except:
            #         idx_quad6 = len(xs)-1
            #     norm_diss_1st = self.normalize(super_smooth_diss_1st)
            #     idx_max_diss1 = idx_quad5 + np.argmax(norm_diss_1st[idx_quad5:idx_quad6])
            #     val_max_diss1 = norm_diss_1st[idx_max_diss1]
            #     val_min_diss1 = np.average(norm_diss_1st[idx_quad5:idx_quad6]) + diss_threshold
            #     norm_diss_2nd = self.normalize(super_smooth_diss_2nd)
            #     idx_max_diss2 = idx_quad5 + np.argmax(norm_diss_2nd[idx_quad5:idx_quad6])
            #     val_max_diss2 = norm_diss_2nd[idx_max_diss2]
            #     val_min_diss2 = np.average(norm_diss_2nd[idx_quad5:idx_quad6]) + diss_threshold
            #     if val_max_diss1 > val_min_diss1 and val_max_diss2 > val_min_diss2:
            #         Log.d("Using DISS dropoff to determine stop point.")
            #         idx_max_diss = int(idx_max_diss1/2+idx_max_diss2/2)
            #         t_blip3 = xs[idx_max_diss]
            #     elif val_max_diss1 > val_min_diss1:
            #         Log.d("Using DISS dropoff to determine stop point.")
            #         t_blip3 = xs[idx_max_diss1]
            #         idx_max_diss = next(x for x,y in enumerate(xs) if y >= t_blip3)
            #     elif val_max_diss2 > val_min_diss2:
            #         Log.d("Using DISS dropoff to determine stop point.")
            #         idx_max_diss = idx_max_diss2
            #         t_blip3 = xs[idx_max_diss2]
            #     else:
            #         idx_max_diss = -1
            # else:
            #     idx_max_diss = -1

            # # pick the closest point FREQ vs DIFF to the DISS point if all are found, but do not agree
            # if (idx_max_freq != -1 and idx_max_diff != -1 and idx_max_diss != -1 and
            #     abs(xs[idx_max_freq] - xs[idx_max_diff]) > max_agree_delta):
            #     Log.w("Multiple end points found, but they do not agree. Determining the best end point...")
            #     if abs(xs[idx_max_diss] - xs[idx_max_freq]) < abs(xs[idx_max_diss] - xs[idx_max_diff]):
            #         Log.w("Using FREQ as it is closest.")
            #         # FREQ is closer, use it, and invalidate DIFF
            #         t_blip3 = xs[idx_max_freq]
            #         idx_max_diff = -1
            #     else:
            #         Log.w("Using DIFF as it is closest.")
            #         # DIFF is closer, use it, and invalidate FREQ
            #         t_blip3 = xs[idx_max_diff]
            #         idx_max_freq = -1
            #     # invalidate DISS, since we used it only to find FREQ vs DIFF comparison
            #     idx_max_diss = -1
            ...
            if start_at != None:
                if len(start_at) == 6:
                    if start_at[0] != None:
                        ...
                        t_start = xs[avg_start]
                        Log.d(f"Using explicit start reference: xs[{avg_start}] = {t_start}s")
                        idx_max_diff = 0 # force t_quadX updates in next section
                    if start_at[-1] != None:
                        if extend_data:
                            avg_stop = next(x for x,y in enumerate(xs) if y >= xs_extend[start_at[-1]])
                        else:
            ...
            # find minima of 1st deriv. in zone region, and scale possibilities to favor those closer to the minima
            idx_quad1 = next(x for x,y in enumerate(xs) if y >= t_quad1)
            ...
            if min_diss_zone2_1 in [0, idx_quad2-idx_quad1-1]:
                min_diss_zone2_1 = -1 # int(idx_quad2/2-idx_quad1/2)
            if min_freq_zone2_1 in [0, idx_quad2-idx_quad1-1]:
                min_freq_zone2_1 = -1 # int(idx_quad2/2-idx_quad1/2)
            if min_diff_zone2_1 in [0, idx_quad2-idx_quad1-1]:
                min_diff_zone2_1 = -1 # int(idx_quad2/2-idx_quad1/2)
            ...
            if min_diss_zone2_2 in [0, idx_quad2-idx_quad1-1]:
                min_diss_zone2_2 = -1 # int(idx_quad2/2-idx_quad1/2)
            if min_freq_zone2_2 in [0, idx_quad2-idx_quad1-1]:
                min_freq_zone2_2 = -1 # int(idx_quad2/2-idx_quad1/2)
            if min_diff_zone2_2 in [0, idx_quad2-idx_quad1-1]:
                min_diff_zone2_2 = -1 # int(idx_quad2/2-idx_quad1/2)
            num_count = 0
            num_sum = 0
            for i in [min_diss_zone2_1, min_freq_zone2_1, min_diff_zone2_1]: #, 
                      #min_diss_zone2_2, min_freq_zone2_2, min_diff_zone2_2]:
                if i != -1:
                    num_sum += i
                    num_count += 1
            if num_count > 0:
                min_peak_zone2 = int(num_sum / num_count) # effectively average
            else:
                min_peak_zone2 = int(idx_quad2/2-idx_quad1/2)
            # min_peak_zone2 = int(np.average([min_diss_zone2, min_freq_zone2, min_diff_zone2]))
            ...
            if max_diss_zone4_1 in [0, idx_quad6-idx_quad5-1]:
                max_diss_zone4_1 = -1 # int(idx_quad6/2-idx_quad5/2)
            if max_freq_zone4_1 in [0, idx_quad6-idx_quad5-1]:
                max_freq_zone4_1 = -1 # int(idx_quad6/2-idx_quad5/2)
            if max_diff_zone4_1 in [0, idx_quad6-idx_quad5-1]:
                max_diff_zone4_1 = -1 # int(idx_quad6/2-idx_quad5/2)
            ...
            if max_diss_zone4_2 in [0, idx_quad6-idx_quad5-1]:
                max_diss_zone4_2 = -1 # int(idx_quad6/2-idx_quad5/2)
            if max_freq_zone4_2 in [0, idx_quad6-idx_quad5-1]:
                max_freq_zone4_2 = -1 # int(idx_quad6/2-idx_quad5/2)
            if max_diff_zone4_2 in [0, idx_quad6-idx_quad5-1]:
                max_diff_zone4_2 = -1 # int(idx_quad6/2-idx_quad5/2)
            ...
            if min_diss_zone4_2 in [0, idx_quad6-idx_quad5-1] or min_diss_zone4_2 < max_diss_zone4_2:
                min_diss_zone4_2 = -1 # int(idx_quad6/2-idx_quad5/2)
            if min_freq_zone4_2 in [0, idx_quad6-idx_quad5-1] or min_freq_zone4_2 < max_freq_zone4_2:
                min_freq_zone4_2 = -1 # int(idx_quad6/2-idx_quad5/2)
            if min_diff_zone4_2 in [0, idx_quad6-idx_quad5-1] or min_diff_zone4_2 < max_diff_zone4_2:
                min_diff_zone4_2 = -1 # int(idx_quad6/2-idx_quad5/2)
            num_count = 0
            num_sum = 0
            for i in [max_diss_zone4_1, max_freq_zone4_1, max_diff_zone4_1, 
                      max_diss_zone4_2, max_freq_zone4_2, max_diff_zone4_2]:
                      #min_diss_zone4_2, min_freq_zone4_2, min_diff_zone4_2]:
                if i != -1:
                    num_sum += i
                    num_count += 1
            if num_count > 0:
                max_peak_zone4 = int(num_sum / num_count) # effectively average
            else:
                max_peak_zone4 = int(idx_quad6/2-idx_quad5/2)
            # max_peak_zone4 = int(np.average(
            #     [max_diss_zone4_1, max_freq_zone4_1, max_diff_zone4_1,
            #      max_diss_zone4_2, max_freq_zone4_2, max_diff_zone4_2]))
            ...
            # normalize now so our scaler multiplication doesn't blow out the signal
            ...
            # normalize again, to bring 1 point back up to 100% certainty (first guess)
            zone1_possibilities = self.normalize_list_of_tuples(zone1_possibilities)
            zone2_possibilities = self.normalize_list_of_tuples(zone2_possibilities)
            zone3_possibilities = self.normalize_list_of_tuples(zone3_possibilities)
            zone4_possibilities = self.normalize_list_of_tuples(zone4_possibilities)

            # add the minima as 60% probability scores, if nothing else looks better in that region
            ...
            if True or end_compare_result == 0:
                ...
                # best_end_idx += max(1, int(best_num_fill_pts / 40))
                best_post_idx = best_end_idx + max(2, int(best_num_fill_pts / 10))
                # for trial_pt in sorted_zone1_possibilities:
                #     if trial_pt == (avg_start, consider_points_above_pct):
                #         continue
                #     if (trial_pt[0] >= best_end_idx + int(best_num_fill_pts / 2) and
                #         trial_pt[0]-best_end_idx < (best_end_idx-best_start_idx) / 2):
                #         best_post_idx = trial_pt[0]
                #         break
            ...
            #     t_start = xs[sorted_zone1_possibilities[0][0]]
            # else:
            #     t_start = (t_quad0 + t_quad1) / 2
                if False:
                    zone1_mags = []
                    for zp in zone1_possibilities:
                        zone1_mags.append(zp[1])
                    avg_zone1_mag = np.average(zone1_mags)
                    for p in zone1_possibilities[::-1]: # unsorted, reversed
                        if p[1] > 0.25: # 2 * avg_zone1_mag:
                            t_start = xs[p[0]]
                            break
                else:
                    min_dist_to_ideal = idx_quad2 - idx_quad1 # max possible distance
            ...
            if len(sorted_zone2_possibilities) > 0:
                if False:
                    last_dist_to_ideal = idx_quad2 - idx_quad1 # max possible distance
                    ...
                    for zp in sorted_zone2_possibilities:
                        ...
                        if dist_to_ideal < min_dist_to_ideal:
                            min_dist_to_ideal = dist_to_ideal
                            min_zp_idx = zp[0]
                # t_blip1 = xs[sorted_zone2_possibilities[0][0]]
            else:
                t_blip1 = (t_quad1 + t_quad2) / 2

            if len(sorted_zone3_possibilities) > 0:
                if False:
                    last_dist_to_ideal = idx_quad4 - idx_quad3 # max possible distance
                    ...
                    min_dist_to_ideal = idx_quad4 - idx_quad3 # max possible distance
            ...
            # ### scale 2nd derivatives in Zone 4 and search for blip larger than 25%, left of current t_blip3
            # ### plotted: self.normalize(np.abs(super_smooth_diss_2nd))[idx_quad5:]
            # maxima_in_zone4 = [x for x in max_idx_2nd if x > idx_quad5 and x < idx_quad6]
            # norm_diss_2nd = self.normalize(np.abs(super_smooth_diss_2nd))
            # max_val_2_zone4 = np.amax(norm_diss_2nd[idx_quad5:idx_quad6])
            # max_time_delta = (xs[idx_quad6] - xs[idx_quad5])*0.20 # 20% of zone4
            # stop_before_xs = t_blip3 - max_time_delta
            # last_pct = -1
            # # max_diff_2_zone4 = np.amax(ys_diff) - np.amin(ys_diff)
            # # Log.d(f"MAX DIFF ZONE 4: {max_diff_2_zone4:2.2f}Hz")
            # if max_val_2_zone4 > 0.05:
            #     for max_z4 in maxima_in_zone4[::-1]:
            #         if xs[max_z4] > t_blip3: continue
            #         if xs[max_z4] < stop_before_xs: break
            #         norm_pct = norm_diss_2nd[max_z4] / max_val_2_zone4
            #         if norm_pct < last_pct and last_pct != -1:
            #             # Log.d(f"NOT Moved END from {t_blip3:2.1f}s to {xs[max_z4]:2.1f}s with {norm_pct*100:2.1f}% less than last")
            #             break
            #         if norm_pct > 0.25:
            #             # Log.d(f"Moved END from {t_blip3:2.1f}s to {xs[max_z4]:2.1f}s with {norm_pct*100:2.1f}% > 25%")
            #             t_blip3 = xs[max_z4]
            #             last_pct = norm_pct
            ...
            # Compute the histogram for Zone 1
            hist_values, bin_edges = np.histogram(xs[x1], bins=n_bins_zone1-1, range=(t_quad0,t_quad1))
            # Associate each 'y' value with the corresponding bin number
            ...
            # for x_idx, x_bin in enumerate(x_bin_numbers):
            #     if x_bin != last_bin:
            #         if bin_possibility_sum != 0:
            #             zone1_confidence.append(bin_possibility_sum)
            #         if (len(zone1_options) >= 3 or 
            #            (x1[x_idx] == ideal_zone1 and
            #             y1[x_idx] == consider_points_above_pct)):
            #             break
            #         zone1_options.append(x1[x_idx])
            #         bin_possibility_sum = y1[x_idx]
            #         last_bin = x_bin
            #     else: # x_bin == last_bin:
            #         bin_possibility_sum += y1[x_idx]
            ...
            if zone1_options[0] != max_bin:
                ...
                max_idx = np.argwhere(zone1_options == max_bin)[0][0] # there must be 1 (and only 1)
            ...
            # Log.d("Zone 1 Timestamps:", xs[zone1_options])
            # Log.d("Zone 1 Options:", zone1_pois)

            # Compute the histogram for Zone 2
            ...
            if zone2_options[0] != max_bin:
                max_idx = np.argwhere(zone2_options == max_bin)[0][0] # there must be 1 (and only 1)
            ...
            # Log.d("Zone 2 Timestamps:", xs[zone2_options])
            # Log.d("Zone 2 Options:", zone2_pois)

            # Compute the histogram for Zone 3
            ...
            if zone3_options[0] != max_bin:
                max_idx = np.argwhere(zone3_options == max_bin)[0][0] # there must be 1 (and only 1)
            ...
            # Log.d("Zone 3 Timestamps:", xs[zone3_options])
            # Log.d("Zone 3 Options:", zone3_pois)

            # Compute the histogram for Zone 4
            ...
            if zone4_options[0] != max_bin:
                max_idx = np.argwhere(zone4_options == max_bin)[0][0] # there must be 1 (and only 1)
            ...
            # Log.d("Zone 4 Timestamps:", xs[zone4_options])
            # Log.d("Zone 4 Options:", zone4_pois)
            ...
            # apply fine tuning to move to the left side of a blip, if one is present at the point of interest
            # must process start_stop, and all 4 zoneX_pois variables
            # NOTE: when start_at provides all 6 points, still process everything above, as ideal_zoneX was set
            ...
            for i in range(len(zone2_pois)):
                ...
                curr_xs_idx = next(x for x,y in enumerate(xs_extend) if y >= xs[curr_xs_idx]) # convert to full dataset
                ...
                while True:
                    ...
                    m2, m3 = abs(m2), abs(m3)
                    end_compare_result = -1 # always move left, regardless of assessment result
                    if m2 < m3 and abs(m2-m0) < abs(m2-m1):
                        end_compare_result = 0 # stop if left side slope is lesser, and closer to 0 than initial fill slope
                    # end_compare_result = self.compare_slopes(m0, m1, m2, m3, end_compare_result)
            ...
            try:
                xs_extend[-1] # will throw if var does not exist
            except:
                xs_extend = xs # create if not already exist
            # convert all found timestamps to indices
            for i in range(len(start_stop)):
                try:
                    start_stop[i] = next(x for x,y in enumerate(xs_extend) if y >= start_stop[i])
                except:
                    start_stop[i] = -1 # not found, put at end of dataset

            # # convert zone pois idx from xs to xs_extend
            # if start_stop[0] > len(xs) - 1 or xs[start_stop[0]] != xs_extend[start_stop[0]]:
            #     for i in range(len(zone1_pois)):
            #         try:
            #             curr_xs_idx = zone1_pois[i][0]
            #             next_xs_idx = next(x for x,y in enumerate(xs_extend) if y >= xs[curr_xs_idx])
            #             same_confid = zone1_pois[i][1]
            #             zone1_pois[i] = (next_xs_idx,same_confid)
            #         except:
            #             Log.e("Failed to extend XS for Zone 1 pois.")
            #             pass
            #     for i in range(len(zone2_pois)):
            #         try:
            #             curr_xs_idx = zone2_pois[i][0]
            #             next_xs_idx = next(x for x,y in enumerate(xs_extend) if y >= xs[curr_xs_idx])
            #             same_confid = zone2_pois[i][1]
            #             zone2_pois[i] = (next_xs_idx,same_confid)
            #         except:
            #             Log.e("Failed to extend XS for Zone 2 pois.")
            #             pass
            #     for i in range(len(zone3_pois)):
            #         try:
            #             curr_xs_idx = zone3_pois[i][0]
            #             next_xs_idx = next(x for x,y in enumerate(xs_extend) if y >= xs[curr_xs_idx])
            #             same_confid = zone3_pois[i][1]
            #             zone3_pois[i] = (next_xs_idx,same_confid)
            #         except:
            #             Log.e("Failed to extend XS for Zone 3 pois.")
            #             pass
            #     for i in range(len(zone4_pois)):
            #         try:
            #             curr_xs_idx = zone4_pois[i][0]
            #             next_xs_idx = next(x for x,y in enumerate(xs_extend) if y >= xs[curr_xs_idx])
            #             same_confid = zone4_pois[i][1]
            #             zone4_pois[i] = (next_xs_idx,same_confid)
            #         except:
            #             Log.e("Failed to extend XS for Zone 4 pois.")
            #             pass

            # num_maxima = 0
            # for idx in maxima_idx_1:
            #     if idx > t_start:
            #         num_maxima += 1

            # num_minima = 0
            # for idx in minima_idx_1:
            #     if idx > t_start:
            #         num_minima += 1
            # Log.i(f"len(maxima) = {num_maxima}")
            # Log.i(f"len(minima) = {num_minima}")

            poi_vals = start_stop

            # insert confidence options for start, blip1, blip2, and blip3
            poi_vals[0] = [(poi_vals[0],zone1_pois[0][1])]
            # leave end-of-fill and post point untouched
            poi_vals[-3] = zone2_pois
            poi_vals[-2] = zone3_pois
            poi_vals[-1] = zone4_pois

            # if starting point was given as max time or -1 (not present), retain that value
            if start_at != None:
                if len(start_at) == 6:
                    for i in range(len(start_at)):
                        ...
                        if poi_idx != None and poi_idx in [-1, len(xs_extend)-1]:
                            ...
                            poi_vals[i] = len(xs_extend)-1 # not present

            ### INSERTED PRIOR CODE ANALYSIS METHOD:
            # start_time = xs_extend[poi_vals[0][0][0]] # next(x for x,y in enumerate(xs_extend) if y >= start_time)
            # # dir = ys[start_time] < 5
            # # while True:
            # #     if ys[start_time] < 5:
            # #         start_time += 1
            ...
            # #         start_time -= 1
            # #         if dir: break
            # # start_time = xs[start_time] # convert index to time
            # # poi1_time = poi_vals[0] # start_time
            # poi2_time = next(x for x,y in enumerate(xs) if y > start_time) # start_time + (fill_time / 128) # new poi
            # while True: # work back in time to find actual minimum difference (true start)
            #     if ys_diff[poi2_time+5] > ys_diff[poi2_time]:
            #         poi2_time += 1
            #     else:
            #         poi2_time -= 2 # move forward 1pt when diff is less than the avg
            #         break
            # poi3_time = xs[poi2_time + 10] # todo: fine tune this point pick
            # poi2_time = xs[poi2_time]
            # poi4_time = start_time + (fill_time / 8) # - (3 * fill_time / 128) # new poi
            # poi5_time = start_time + (fill_time / 2) # - (fill_time / 64) # new poi
            # # poi6_time = stop_time
            # t05 = start_time + (fill_time * 0.05) # 05%
            # t20 = start_time + (fill_time * 0.20) # 20%
            # t40 = start_time + (fill_time * 0.40) # 40%
            # t60 = start_time + (fill_time * 0.60) # 60%
            # t05percent = next(x for x,y in enumerate(xs) if y > t05)
            # t20percent = next(x for x,y in enumerate(xs) if y > t20)
            # t40percent = next(x for x,y in enumerate(xs) if y > t40)
            # t60percent = next(x for x,y in enumerate(xs) if y > t60)
            # zeros4 = np.where(np.diff(np.sign(ys_diss_2nd)))[0]
            # poi4_search_region = ys_diss_2nd[t05percent:t20percent]
            # poi4_search_max = t05percent + np.argmax(poi4_search_region)
            # poi4_found_time = next(y for y in zeros4 if y > poi4_search_max)
            # poi5_search_region = ys_diss_2nd[t40percent:t60percent]
            # poi5_search_max = t40percent + np.argmax(poi5_search_region)
            # poi5_found_time = next(y for y in zeros4 if y > poi5_search_max)
            # poi4_time = xs[poi4_found_time]
            # poi5_time = xs[poi5_found_time]
            # Log.w(f"Auto-detected points: poi4 = {poi4_time}, poi5 = {poi5_time}")
            # poi_vals[-3] = next(x for x,y in enumerate(xs_extend) if y > poi4_time)
            # poi_vals[-2] = next(x for x,y in enumerate(xs_extend) if y > poi5_time)
            ### END: PRIOR CODE ANALYSIS METHOD

            # Determine if run still looks good
            min_freq = np.argmin(resonance_frequency[t_0p5:]) # ignore starting baseline region
            ...
            if t_blip3 - t_fill_start < 3.0:
                ...
                is_good = False
            if min_freq < avg_start - t_0p5: # freq goes down once drop is applied
                Log.e(TAG, "Bad run due to 'resonance_frequency' minimum being after run start.")
                is_good = False
            if max_diss < avg_start - t_0p5: # diss goes up once drop is applied
                Log.e(TAG, "Bad run due to 'dissipation' minimum being after run start.")
                is_good = False
                ...
                if len(start_at) == 6:
                    if start_at[-1] != None and start_at[-1] not in [len(xs_extend-1)-1, -1]:
                        if xs_factor_d < 75:
                            ...
                            xs_factor_d = 100 # prevent partial run detection
            if xs_factor_d < 75:
            ...
            # d1_offset_begin = -50
            # d1_offset_end = +250
            # deriv_smoother = PolynomialSmoother(degree=7, copy=False)
            # deriv_smoother.smooth(ys_diss_1st[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end])

            # # Method: Use polyfit to calculate coeffs
            # coeffs = np.polyfit(xs[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end], 
            #                     ys_diss_1st[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end],
            #                     7)
            
            # print(f"ROOTS = {xs[zeros3]}")
            # all_coeffs = self.find_all_polys(degree=7, roots=xs[zeros3])
            # all_d1_fit = []
            # for coeffs in all_coeffs:
            #     all_d1_fit.append(np.polyval(coeffs, xs))

            # operate curve_fit on 1st deriv. dissipation data
            # init = [ 2.24681918e-11, -1.26401977e-08,  2.30418962e-06, -1.98880042e-04,
            #          1.94086531e-02,  4.06326664e-01,  5.02741962e+01, -8.28286684e+02,
            #          6.91452206e-01,  6.03650931e-01,  2.22739315e-01,  1.27356065e+01,
            #         -7.81441061e+00, -5.93585305e+01, -1.26230367e+01,  1.00000000e+00]
            # init = [ 4.47035476e-10, -1.75058662e-08,  2.87201682e-06, -2.08085983e-04,
            #          2.14623014e-02,  4.24634636e-01,  5.05056312e+01, -8.29201810e+02,
            #         -3.56482980e+01, -6.55683322e+00, -4.92722581e+00,  1.23653926e+01,
            #         -7.56865558e+00, -5.89020941e+01, -1.26314924e+01,  1.00000000e+00]
            # init = np.array(all_coeffs[2],dtype='float64')
            # param, param_cov = curve_fit(self.test_septic_func_fit, 
            #                              xs[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end], 
            #                              self.normalize(ys_diss_1st[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end]),
            #                              init)
            # 
            # print(f"FIT = {param}, {param_cov}")
            # # a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p = param
            ...
            if poi_vals[0] > len(xs) - 1 or xs[poi_vals[0]] != xs_extend[poi_vals[0]]:
                for i in range(len(poi_vals)):
                    # poi_vals[i] = int(poi_vals[i] / Constants.downsample_file_count)
            ...
            if not self._plt_init:
                ...
                mngr.window.setGeometry(0,0,1920,1080) # (0,-1080,1920,1080)
            else:
                self.ax1.cla()
                self.ax2.cla()
                self.ax3.cla()
            self.ax1.set_title(data_title)
            self.ax1.plot(xs[poi_vals], self.normalize(ys[poi_vals]), 'go') # pois
            for p in poi_vals:
                self.ax1.axvline(xs[p], color='blue', linestyle='-')

            # self.ax1.plot(xs, self.normalize(ys_fit), 'r-')                 # fit dissipation
            # self.ax1.plot(xs, smoother.data[0], color='black')
            # self.ax1.plot(xs, ys, 'r.')
            ...
            # self.ax2.plot(xs, self.normalize(np.abs(accumulated_1st), 0.5, 1), 'g-')           # 1st deriv. (dissipation)
            # self.ax2.plot(xs[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end], 
            #               self.normalize(deriv_smoother.smooth_data[0]), linewidth=1, color='green')
            # self.ax2.plot(xs[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end], 
            #               self.test_septic_func_fit(xs[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end], 
            #               a,b,c,d,e,f,g,h)) # ,i,j,k,l,m,n,o,p))
            # self.ax2.plot(xs[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end], 
            #               np.polyval(coeffs, xs[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end]))
            # for data in all_d1_fit:
            #     self.ax2.plot(xs[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end], 
            #                   self.normalize(data[poi_vals[0]+d1_offset_begin:poi_vals[-1]+d1_offset_end]))
            # normalized_ys_comb_1st_avg = np.average(self.normalize(np.abs(accumulated_1st), 0.5, 1))
            # normalized_ys_comb_2nd_avg = np.average(self.normalize(np.abs(accumulated_2nd), 0, 0.5))
            # normalized_ys_comb_1st_avg = np.amax(self.normalize(accumulated_1st, 0, 0.5)[t_0p5:t_1p0]) + 0.5
            # normalized_ys_comb_2nd_avg = np.amax(self.normalize(accumulated_2nd, 0, 0.5)[t_0p5:t_1p0])
            # self.ax2.axhline(normalized_ys_comb_1st_avg, color='green', linestyle=":")
            # ys_diss_diff_sm = self.normalize(savgol_filter(ys_diss_1st, 9, 1), 0.5, 1)
            # self.ax2.plot(xs, ys_diss_diff_sm, "g:")
            # self.ax2.plot(xs, self.normalize(super_smooth_1st_deriv, 0.5, 1), "g:")
            # self.ax2.plot(xs[maxima_idx_1], self.normalize(maxima_val_1, 0.5, 1), "g.")
            # self.ax2.axvline(t_quad0, color='black', linestyle=":")
            # self.ax2.axvline(t_quad1, color='black', linestyle=":")
            # self.ax2.axvline(t_quad2, color='black', linestyle=":")
            # self.ax2.axvline(t_quad3, color='black', linestyle=":")
            # self.ax2.axvline(t_quad4, color='black', linestyle=":")
            self.ax2.plot(xs, self.normalize(np.abs(accumulated_2nd), 0, 1), 'b-')           # 2nd deriv. (dissipation)
            # self.ax2.axhline(normalized_ys_comb_2nd_avg, color='blue', linestyle=":")
            # self.ax2.axhline(avg_seq_mag, color='red', linestyle=":")

            # disable these zones to see end zone better
            ...
            # self.ax3.plot(xs, self.normalize(rought_fit), color='pink')

            # self.ax3.plot(xs, adjusted_diss_1st, color='orange')
            # self.ax3.plot(xs, adjusted_freq_1st, color='yellow')
            # self.ax3.plot(xs, adjusted_diff_1st, color='purple')
            # self.ax3.plot(xs, accumulated_1st, color='pink')

            init_fill = False

            #ignore np.abs(), so set idx_quad5 to fake it into not showing it
            # idx_quad5 = 0 if init_fill else -1
            ...
            # self.ax3.axvline(expected_end1, color='black', linestyle=":")
            # self.ax3.fill_betweenx([0,1], expected_end1, expected_end2, alpha=0.1, color='yellow')
            # self.ax3.axvline(expected_end2, color='black', linestyle=":")
            # self.ax3.axhline(avg_diss_1/100, color='black', linestyle="-")
            # self.ax3.axhline(half_diss_1/100, color='black', linestyle="-")
            ...
            # self.ax3.plot(xs[idx_end1:idx_end2], self.normalize(np.abs(super_smooth_diff_2nd[idx_end1:idx_end2])), color='pink', linestyle='dashdot')
            ...
            if init_fill:
                ...
                # self.ax3.axhline(diff_baseline, color="black", alpha=0.5)
                ...
                xlim_min = xs_extend[debug_point1] - 1 # t_quad0 - 1.0 # xs[debug_point1 - 10]
                xlim_max = xs_extend[best_post_idx] + 1 # t_quad1 + 1.0 # xs[debug_point2 + 10]
                ylim_min = -0.05 # break_diff_low - 0.05
                ylim_max = np.amax(ys_diff_smooth_fill[debug_point1:best_post_idx]) + 0.05 # 1.05 # break_diff_high + 0.05
        ...
        return poi_vals
    # def find_all_polys(self, degree, roots):
    ...
    #                 term *= (x-root)
    #             poly = Poly(term.expand(), x)
    #             print(f'f(x) = {term.expand()}')
    #             ret_coeffs.append(poly.all_coeffs())
    #         return ret_coeffs
    #     else:
    #         raise ValueError('The root list should not contain duplicated roots')
    # def test_septic_func_fit(self, x, a, b, c, d, e, f, g, h):
    #     i=j=k=l=m=n=o=p=0
    #     return (a * np.power(x + i, 7) + b * np.power(x + j, 6) + c * np.power(x + k, 5) + d * np.power(x + l, 4) +
    #             e * np.power(x + m, 3) + f * np.power(x + n, 2) + g * np.power(x + o, 1) + h * np.power(x + p, 0))
    def run(self):
        plt.ion()
        # path_root = "content/testing-partial-fills/"
