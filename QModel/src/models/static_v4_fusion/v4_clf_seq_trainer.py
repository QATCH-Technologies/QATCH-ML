...
class POIDetectionModel(nn.Module):
    ...
    POI4, POI5, POI6 predictions are conditioned on previous POIs.
    """

    def __init__(self, window_size: int, feature_dim: int,
                 conv_filters_1: int = 64, conv_filters_2: int = 128,
                 kernel_size: int = 3, lstm_units_1: int = 128,
                 lstm_units_2: int = 64, dense_units: int = 128,
                 dropout_1: float = 0.3, dropout_2: float = 0.3,
                 dropout_3: float = 0.3, use_sequential_structure: bool = True):
        ...
        if self.use_sequential_structure:
            ...
            self.fc_non_poi = nn.Linear(dense_units, 1)

            # POI4 head (independent, comes first)
            self.fc_poi4_hidden = nn.Linear(dense_units, dense_units // 2)
            self.fc_poi4 = nn.Linear(dense_units // 2, 1)

            # POI5 head (conditioned on POI4)
            # Takes: shared features + POI4 hidden representation
            self.fc_poi5_hidden = nn.Linear(
                dense_units + dense_units // 2, dense_units // 2)
            self.fc_poi5 = nn.Linear(dense_units // 2, 1)

            # POI6 head (conditioned on POI5)
            # Takes: shared features + POI5 hidden representation
            self.fc_poi6_hidden = nn.Linear(
                dense_units + dense_units // 2, dense_units // 2)
            ...
            self.fc_output = nn.Linear(dense_units, 4)
...
class POIDetectionSystem:
    ...
    def _create_windows_and_multilabels(self, features_df: pd.DataFrame,
        ...
        # Map POI positions to their indices (only care about POI 4, 5, 6)
        poi_map = {
    ...
    def tune(self, X: np.ndarray, y: np.ndarray, n_trials: int = 20,
             timeout: int = None, epochs_per_trial: int = 15) -> optuna.Study:
        """
    ...
    def visualize_predictions(self, df: pd.DataFrame, save_path: str = None,
        ...
        # Get predictions
        predicted_pois_full = self.predict_pois(df)
        predicted_pois_no_constraints = self.predict_pois(df)
        ...
        for i in range(0, n_samples, self.stride):
            ...
            # Pad with last value if not enough left
            ...
            # Compute center position (clamped)
            pos = min(i + self.window_size // 2, n_samples - 1)
            window_positions.append(pos)

            # Handle x-values (time or index)
...
if __name__ == "__main__":
    # Initialize system with sequential structure enforcement
    poi_system = POIDetectionSystem(
        window_size=128,
        stride=16,
        tolerance=64,
        use_sequential_structure=True  # Enable architectural POI4→POI5→POI6 enforcement
    )
    ...
    # # - Balanced: n_trials=100, epochs_per_trial=20 (RECOMMENDED)
    # # - Thorough: n_trials=200, epochs_per_trial=30
    # study = poi_system.tune(X, y, n_trials=5, epochs_per_trial=20)
