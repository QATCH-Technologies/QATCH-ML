...
class POIBenchmark:
    ...
    def __init__(self):
        ...
        self.predictor_names = []  # Track predictor names dynamically
    ...
    def benchmark(self, data_dir: str, num_datasets: int,
                  fusion_predictor_1: QModelV4Fusion,
                  fusion_predictor_2: QModelV4Fusion,
                  v4_predictor: Optional[QModelPredictorV4] = None,
                  fusion_kwargs_1: Optional[Dict] = None,
                  fusion_kwargs_2: Optional[Dict] = None,
                  v4_kwargs: Optional[Dict] = None):
        """Run benchmark on two V4Fusion predictors and optionally V4Predictor."""
        fusion_kwargs_1 = fusion_kwargs_1 or {}
        fusion_kwargs_2 = fusion_kwargs_2 or {}
        ...
        for i, (data_file, poi_file) in enumerate(dataset_pairs, 1):
            ...
            # Run V4 Fusion Original
            result_fusion_1 = self.run_predictor(
                fusion_predictor_1, data_file, "V4_Fusion_Original", poi_file, **fusion_kwargs_1
            )
            self.results.append(result_fusion_1)
            print(
    ...
    def plot_results(self, index_df: pd.DataFrame, time_df: pd.DataFrame,
        ...
        mae_pivot.plot(kind='bar', ax=ax1, color=colors)
        ...
        rmse_pivot.plot(kind='bar', ax=ax2, color=colors)
        ...
        detection_pivot.plot(kind='bar', ax=ax3, color=colors)
        ...
        time_mae_pivot.plot(kind='bar', ax=ax4, color=colors)
        ...
        time_rmse_pivot.plot(kind='bar', ax=ax5, color=colors)
        ...
        time_mse_pivot.plot(kind='bar', ax=ax6, color=colors)
        ...
        norm_mae_pivot.plot(kind='bar', ax=ax7, color=colors)
        ...
        norm_rmse_pivot.plot(kind='bar', ax=ax8, color=colors)
        ...
        norm_mse_pivot.plot(kind='bar', ax=ax9, color=colors)
        ...
        for predictor_name in self.predictor_names:
            ...
            if all_norm_errors:
                ...
                display_name = predictor_name.replace('_', ' ')
        ...
        if comparison_data:
            ...
            for predictor in self.predictor_names:
                ...
                color = color_map.get(predictor, '#666666')
                ax12.scatter(pred_data['Absolute_Error'], pred_data['Normalized_Error'],
                             alpha=0.5, label=predictor.replace('_', ' '), color=color, s=30)
        ...
        fig.suptitle('POI Predictor Benchmark - Dual V4 Fusion Comparison',
    ...
    def generate_report(self, index_df: pd.DataFrame, time_df: pd.DataFrame,
        ...
        report.append("="*80)
        report.append(
            "POI PREDICTOR BENCHMARK REPORT - DUAL V4 FUSION COMPARISON")
        report.append("="*80)
        report.append("")
...
if __name__ == "__main__":
    ...
    # V4 Fusion with original classifier
    fusion_predictor_original = QModelV4Fusion(
        reg_path_1=FUSION_REG_PATH_1,
        reg_path_2=FUSION_REG_PATH_2,
        clf_path=FUSION_CLF_PATH_ORIGINAL,
        reg_batch_size=512,
        clf_batch_size=256
    )
    ...
    # Optional: V4 Predictor (set to None to skip)
    ...
    benchmark.benchmark(
        ...
        v4_kwargs={'top_k': 3,
                   'apply_constraints': True} if v4_predictor else None
    )
    ...
    benchmark.generate_report(
        ...
        time_df,
        normalized_df,
        output_path="benchmark_report_dual_fusion.txt"
    )
    ...
    benchmark.plot_results(
        ...
        time_df,
        normalized_df,
        save_path="benchmark_results_dual_fusion.png"
    )

    # Save metrics to CSV
    index_df.to_csv("benchmark_index_metrics_dual.csv", index=False)
    time_df.to_csv("benchmark_time_metrics_dual.csv", index=False)
    normalized_df.to_csv("benchmark_normalized_metrics_dual.csv", index=False)
