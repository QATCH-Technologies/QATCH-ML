...
import joblib
from QConstants import S_TARGETS, TRAINING, FEATURES, TESTING, PLOTTING
...
def xgb_pipeline(training_content: list) -> pd.DataFrame:
    ...
    for f in tqdm(training_content, desc="<<Processing XGB>>"):
        if f.endswith(".csv") and not f.endswith("_poi.csv"):
            qdp_pipeline = QDataPipeline(f, multi_class=False)
            ...
            if (
                ...
                indices = qdp_pipeline.__dataframe__.index[
                    qdp_pipeline.__dataframe__["Class_6"] != 0
                ].tolist()
    ...
    resampled_df = resample_df(xgb_df, S_TARGETS[3], S_TARGETS)
    print(f"[INFO] Collected {dataset_count} for training.")
    return resampled_df


if __name__ == "__main__":
    print("[INFO] q_train_single.py script start")

    model_name = f"QSingle4_long"
    ...
    if TRAINING:
        ...
        print("[STATUS] Building single-target, long end model")
        qmodel = QModel(
            dataset=training_set, predictors=FEATURES, target_features=S_TARGETS[3]
        )
        qmodel.tune()
        qmodel.train_model()
    ...
    if TESTING:
        ...
        TEST_PATH = f"content/test_data/test"
        ...
        for filename in tqdm(content, desc="<<Testing on files>>"):
            if (
                ...
                if time_delta == -1:
                    continue
                print("[INFO] Predicting using single-target, long end model")
                predictions = qmp.predict(qdp, time_delta)
                pois = []
