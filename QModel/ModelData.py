...
class ModelData:
    ...
    def IdentifyPoints(
        ...
        diss=None,
        start_at=None,
        sf=Constants.super_smooth_factor_ratio,
        i=-1,
        actual=[],
    ):
        xs = ys = [0]
        ...
        try:
            ...
            data_title = "{}_{}".format(
                int(i / 2) + 1, os.path.splitext(os.path.basename(data_path))[0]
            )
            ...
            if delta_freq < 200:
                Log.e(
                    TAG, "Bad run due to 'resonance_frequency' delta being too small."
                )
                is_good = False
            if delta_diss < 1e-5:
            ...
            if extend_data:
                ...
                if after > 5 * before:
                    ...
                    Log.w(
                        "This data does not appear to be downsampled. Treating entire run as regular data, downsampling all of it."
                    )
                    t_first_90_split = -1
                    extend_data = True
            if extend_data:
            ...
            if extend_data:
                ...
                ys_freq = np.concatenate(
                    (ys_freq[0:t_first_90_split:20], ys_freq[t_first_90_split:])
                )
            ...
            ys_freq_fit = savgol_filter(
                ys_freq, extend_smf if extend_data else smooth_factor, 1
            )
            ys_diff_fit = savgol_filter(
                ys_diff, extend_smf if extend_data else smooth_factor, 1
            )
            if extend_data:
            ...
            super_smoother = ConvolutionSmoother(
                window_len=10 * smooth_factor, window_type="ones"
            )
            super_smoother.smooth(ys)
            super_smooth_diss_1st = self.normalize(
                savgol_filter(super_smoother.smooth_data[0], 11, 1, 1), 0, 100
            )
            super_smooth_diss_1st = super_smoother.smooth(
                super_smooth_diss_1st
            ).smooth_data[0]
            super_smooth_diss_2nd = savgol_filter(super_smooth_diss_1st, 11, 1, 1)
            super_smooth_diss_2nd = super_smoother.smooth(
                super_smooth_diss_2nd
            ).smooth_data[0]
            super_smooth_freq_1st = self.normalize(
                savgol_filter(super_smoother.smooth(ys_freq).smooth_data[0], 11, 1, 1),
                0,
                100,
            )
            super_smooth_freq_1st = super_smoother.smooth(
                super_smooth_freq_1st
            ).smooth_data[0]
            super_smooth_freq_2nd = savgol_filter(super_smooth_freq_1st, 11, 1, 1)
            super_smooth_freq_2nd = super_smoother.smooth(
                super_smooth_freq_2nd
            ).smooth_data[0]
            super_smooth_diff_1st = self.normalize(
                savgol_filter(super_smoother.smooth(ys_diff).smooth_data[0], 11, 1, 1),
                0,
                100,
            )
            super_smooth_diff_1st = super_smoother.smooth(
                super_smooth_diff_1st
            ).smooth_data[0]
            super_smooth_diff_2nd = savgol_filter(super_smooth_diff_1st, 11, 1, 1)
            super_smooth_diff_2nd = super_smoother.smooth(
                super_smooth_diff_2nd
            ).smooth_data[0]
            ...
            try:
                max_val_1st = next(
                    y for y in max_idx_1st if super_smooth_diss_1st[y] > avg_diss_1
                )
                min_val_2nd = next(
                    max_idx_2nd[max(0, x - 1)]
                    for x, y in enumerate(max_idx_2nd)
                    if y > max_val_1st
                )
                max_val_2nd = next(y for y in min_idx_2nd if y > max_val_1st)
                half_diss_1 = (
                    avg_diss_1 + (super_smooth_diss_1st[max_val_1st] - avg_diss_1) * 0.4
                )
                min_val_1st = next(
                    y
                    for y in min_idx_1st
                    if y > max_val_2nd and super_smooth_diss_1st[y] < half_diss_1
                )
            except Exception as e:
                max_val_1st = 0
                min_val_2nd = 0
            ...
            if expected_end1 == xs[-1]:
                Log.w(
                    "This run looks like an incomplete fill based on the initial fill duration!"
                )
            ...
            try:
                avg_stop1 = idx_end1 + np.argmax(
                    np.abs(super_smooth_diff_2nd[idx_end1:idx_end2])
                )
            except:
                avg_stop1 = len(xs) - 1
            try:
            ...
            ys_diss_1st = savgol_filter(
                ys_fit, extend_smf if extend_data else smooth_factor, 1, 1
            )
            ...
            ys_diss_2nd = savgol_filter(
                ys_diss_1st, extend_smf if extend_data else smooth_factor, 1, 1
            )
            ...
            ys_freq_1st = savgol_filter(
                ys_freq_fit, extend_smf if extend_data else smooth_factor, 1, 1
            )
            deriv_smoother.smooth(ys_freq_1st)
            ys_freq_1st = np.abs(deriv_smoother.smooth_data[0])
            ys_freq_2nd = savgol_filter(
                ys_freq_1st, extend_smf if extend_data else smooth_factor, 1, 1
            )
            ...
            ys_diff_1st = savgol_filter(
                ys_diff_fit, extend_smf if extend_data else smooth_factor, 1, 1
            )
            deriv_smoother.smooth(ys_diff_1st)
            ys_diff_1st = np.abs(deriv_smoother.smooth_data[0])
            ys_diff_2nd = savgol_filter(
                ys_diff_1st, extend_smf if extend_data else smooth_factor, 1, 1
            )
            ...
            if extend_data:
                ...
                low3_fill, up3_fill = data_smoother.get_intervals(
                    "sigma_interval", n_sigma=1
                )
            else:
            ...
            baseline_diss_1st = savgol_filter(
                adjusted_diss_1st, Constants.baseline_smooth, 1
            )
            ...
            baseline_freq_1st = savgol_filter(
                adjusted_freq_1st, Constants.baseline_smooth, 1
            )
            ...
            baseline_diff_1st = savgol_filter(
                adjusted_diff_1st, Constants.baseline_smooth, 1
            )
            ...
            accumulated_1st = (
                self.normalize(adjusted_diss_1st)
                + self.normalize(adjusted_freq_1st)
                + self.normalize(adjusted_diff_1st)
            )
            minima_idx_1 = argrelextrema(accumulated_1st, np.less)[0]
            minima_val_1 = accumulated_1st[minima_idx_1]
            minima_dict_1 = {
                minima_idx_1[i]: minima_val_1[i] for i in range(len(minima_idx_1))
            }
            ...
            maxima_dict_1 = {
                maxima_idx_1[i]: maxima_val_1[i] for i in range(len(maxima_idx_1))
            }
            maxima_sort_1 = sorted(
                maxima_dict_1.items(), key=lambda kv: (kv[1], kv[0]), reverse=True
            )
            if debug_plots:
                plt.figure()
            ...
            adjusted_diss_2nd = np.concatenate(
                (np.zeros(t_1p0), ys_diss_2nd_abs[t_1p0:])
            )
            baseline_diss_2nd = savgol_filter(
                adjusted_diss_2nd, Constants.baseline_smooth, 1
            )
            ...
            baseline_freq_2nd = savgol_filter(
                adjusted_freq_2nd, Constants.baseline_smooth, 1
            )
            ...
            baseline_diff_2nd = savgol_filter(
                adjusted_diff_2nd, Constants.baseline_smooth, 1
            )
            ...
            accumulated_2nd = (
                accumulated_1st
                + self.normalize(adjusted_diss_2nd)
                + self.normalize(adjusted_freq_2nd)
                + self.normalize(adjusted_diff_2nd)
            )
            minima_idx_2 = argrelextrema(accumulated_2nd, np.less)[0]
            minima_val_2 = accumulated_2nd[minima_idx_2]
            minima_dict_2 = {
                minima_idx_2[i]: minima_val_2[i] for i in range(len(minima_idx_2))
            }
            ...
            maxima_dict_2 = {
                maxima_idx_2[i]: maxima_val_2[i] for i in range(len(maxima_idx_2))
            }
            maxima_sort_2 = sorted(
                maxima_dict_2.items(), key=lambda kv: (kv[1], kv[0]), reverse=True
            )
            if debug_plots:
                plt.figure()
            ...
            level_smooth_factor = max(
                3, int(level_smooth_factor) + (int(level_smooth_factor + 1) % 2)
            )
            level_smoother = ConvolutionSmoother(
                window_len=5 * level_smooth_factor, window_type="ones"
            )
            super_smooth_diss_t1 = savgol_filter(
                level_smoother.smooth(ys_fit).smooth_data[0], 11, 1
            )
            super_smooth_freq_t1 = savgol_filter(
                level_smoother.smooth(ys_freq_fit).smooth_data[0], 11, 1
            )
            super_smooth_diff_t1 = savgol_filter(
                level_smoother.smooth(ys_diff_fit).smooth_data[0], 11, 1
            )
            if debug_plots:
                plt.figure()
            ...
            sorted_possibilities = sorted(
                sorted_possibilities, key=lambda kv: (kv[1], kv[0]), reverse=True
            )
            sequential_possibilities = sorted(
                sorted_possibilities, key=lambda kv: (kv[0], kv[1]), reverse=False
            )
            ...
            if len(sequential_possibilities) > 0:
                ...
                while True:
                    ...
                    if t_start < min_allowed_start:
                        Log.w(
                            "WARN: It looks like the run may have started prior to 3 seconds (during baseline period)."
                        )
                    if (
                        min_allowed_start < t_start < xs[max_allowed_start]
                        and t_blip3 > xs[min_allowed_stop]
                    ):
                        if first_loop:
                            break
                        allow_break = True
                    ...
                    if time.time() > timeout_at:
                        Log.e(
                            "Model took too long to finish and was aborted. Please manually select points."
                        )
                        break
            ...
            while True:
                error_at_this_point = (
                    up3_fill[0][this_idx - 10] - low3_fill[0][this_idx - 10]
                ) / 2
                ...
                if this_idx == idx_quad0:
                    break
                this_idx -= 1
            break_diff_low = ys_diff_smooth_fill[this_idx] - 1.5 * (
                ys_diff_smooth_fill[this_idx] - low3_fill[0][this_idx]
            )
            break_diff_high = ys_diff_smooth_fill[this_idx] + 3 * (
                up3_fill[0][this_idx] - ys_diff_smooth_fill[this_idx]
            )
            while True:
                if (
                    norm_ys_diff[this_idx] > break_diff_high
                    and start_diff < this_idx - 1
                ):
                    ...
                    if norm_ys_diff[start_diff + start_argmin] < break_diff_low:
                        Log.d(
                            "NOTICE: DIFF curve drops from baseline at start of fill."
                        )
                        method = 4
                        ...
                        while True:
                            if norm_ys_diff[this_idx - 1] > norm_ys_diff[this_idx]:
                                Log.d(
                                    "NOTICE: Found start point using relative minima approximation"
                                )
                                method = 1
                                start_diff = this_idx
                                break
                            if norm_ys_diff[this_idx] < up3_fill[0][debug_point1]:
                                Log.d(
                                    "NOTICE: Found start point at crossing of average baseline"
                                )
                                method = 2
                                start_diff = this_idx
                                break
                            if this_idx <= debug_point1:
                                Log.w(
                                    "WARN: Points are ever increasing at baseline DIFF, using 'start-1'"
                                )
                                method = 3
            ...
            while True:
                ...
                post_pointfit = (m2, b2)
                x_intersection, y_intersection = self.find_intersect(
                    max_pointfit, post_pointfit
                )
                end_idx = next(
                    x for x, y in enumerate(xs_extend) if y >= x_intersection
                )
                post_idx = max_idx
                ...
                while True:
                    ...
                    window_size = 1
                    p4 = (
                        xs_extend[this_idx - window_size],
                        norm_ys_diff_soft[this_idx - window_size],
                    )
                    ...
                    end_compare_result = self.compare_slopes(
                        m1, m2, left_slope, right_slope, end_compare_result
                    )
                ...
                if end_compare_result != 0 and num_attempts < 2:
                    Log.w(
                        "Failed to find matching slope point! Using approximated end-of-fill point."
                    )
                if num_attempts >= 1:
                    if best_end_idx != 0:
                        this_diff_mag_delta = (
                            norm_ys_diff_fit[end_idx] - norm_ys_diff_fit[start_idx]
                        )
                        last_diff_mag_delta = (
                            norm_ys_diff_fit[best_end_idx]
                            - norm_ys_diff_fit[best_start_idx]
                        )
                        if (
                            end_idx - start_idx < best_end_idx - best_start_idx
                            or this_diff_mag_delta < last_diff_mag_delta
                        ):
                            pass
                        else:
            ...
            if extend_data:
                avg_start = next(
                    x for x, y in enumerate(xs) if y >= xs_extend[best_start_idx]
                )
            else:
            ...
            if len(norm_freq_fit[i_end_search:]) < 5:
                delta_freq = 0.95
            if (
                t_max_search < xs[idx_max_freq] < t_quad6
                and norm_freq_fit[-1] < norm_freq_fit[idx_max_freq] * delta_freq
            ):
                pass
            else:
                idx_max_freq = -1
            ...
            if len(norm_diff_fit[i_end_search:]) < 5:
                delta_diff = 0.95
            if (
                t_max_search < xs[idx_max_diff] < t_quad6
                and norm_diff_fit[-1] < norm_diff_fit[idx_max_diff] * delta_diff
            ):
                pass
            else:
                idx_max_diff = -1
                    ...
                    if start_at[0] != None:
                        if extend_data:
                            avg_start = next(
                                ...
                                if y >= xs_extend[start_at[0]]
                            )
                        else:
                            avg_start = start_at[0]
                        t_start = xs[avg_start]
                        Log.d(
                            f"Using explicit start reference: xs[{avg_start}] = {t_start}s"
                        )
                        idx_max_diff = 0
                    if start_at[-1] != None:
                        if extend_data:
                            avg_stop = next(
                                x
                                for x, y in enumerate(xs)
                                if y >= xs_extend[start_at[-1]]
                            )
                        else:
                            avg_stop = start_at[-1]
                        t_blip3 = xs[avg_stop]
                        Log.d(
                            f"Using explicit stop reference: xs[{avg_stop}] = {t_blip3}s"
                        )
                        idx_max_diff = 0
                else:
                    Log.w(
                        "Starting conditions (start_at) are the wrong length (!= 6). Ignoring them."
                    )
            ...
            min_peak_zone3 = int(
                np.average([min_diss_zone3, min_freq_zone3, min_diff_zone3])
            )
            ...
            if (
                min_diss_zone4_2 in [0, idx_quad6 - idx_quad5 - 1]
                or min_diss_zone4_2 < max_diss_zone4_2
            ):
                min_diss_zone4_2 = -1
            if (
                min_freq_zone4_2 in [0, idx_quad6 - idx_quad5 - 1]
                or min_freq_zone4_2 < max_freq_zone4_2
            ):
                min_freq_zone4_2 = -1
            if (
                min_diff_zone4_2 in [0, idx_quad6 - idx_quad5 - 1]
                or min_diff_zone4_2 < max_diff_zone4_2
            ):
                min_diff_zone4_2 = -1
            num_count = 0
            num_sum = 0
            ...
            if start_at != None:
                if len(start_at) == 6:
                    if start_at[0] != None:
                        ...
                        Log.d(
                            f"Ideal zone 1 point: xs[{ideal_zone1}] = {xs[ideal_zone1]}s"
                        )
                    if start_at[-3] != None:
                        if extend_data:
                            ideal_zone2 = next(
                                x
                                for x, y in enumerate(xs)
                                if y >= xs_extend[start_at[-3]]
                            )
                        else:
                            ideal_zone2 = start_at[-3]
                        Log.d(
                            f"Ideal zone 2 point: xs[{ideal_zone2}] = {xs[ideal_zone2]}s"
                        )
                    if start_at[-2] != None:
                        if extend_data:
                            ideal_zone3 = next(
                                x
                                for x, y in enumerate(xs)
                                if y >= xs_extend[start_at[-2]]
                            )
                        else:
                            ideal_zone3 = start_at[-2]
                        Log.d(
                            f"Ideal zone 3 point: xs[{ideal_zone3}] = {xs[ideal_zone3]}s"
                        )
                    if start_at[-1] != None:
                        if extend_data:
                            ideal_zone4 = next(
                        ...
                        Log.d(
                            f"Ideal zone 4 point: xs[{ideal_zone4}] = {xs[ideal_zone4]}s"
                        )
                else:
                    Log.w(
                        "Starting conditions (start_at) are the wrong length (!= 6). Ignoring them."
            ...
            sorted_zone1_possibilities = sorted(
                zone1_possibilities, key=lambda kv: (kv[1], kv[0]), reverse=True
            )
            sorted_zone2_possibilities = sorted(
                zone2_possibilities, key=lambda kv: (kv[1], kv[0]), reverse=True
            )
            sorted_zone3_possibilities = sorted(
                zone3_possibilities, key=lambda kv: (kv[1], kv[0]), reverse=True
            )
            sorted_zone4_possibilities = sorted(
                zone4_possibilities, key=lambda kv: (kv[1], kv[0]), reverse=True
            )
            ...
            t_fill_start = (
                xs_extend[best_start_idx] if extend_data else xs[best_start_idx]
            )
            ...
            hist_values, bin_edges = np.histogram(
                xs[x1], bins=n_bins_zone1 - 1, range=(t_quad0, t_quad1)
            )
            ...
            hist_values, bin_edges = np.histogram(
                xs[x2], bins=n_bins_zone2 - 1, range=(t_quad1, t_quad2)
            )
            ...
            hist_values, bin_edges = np.histogram(
                xs[x3], bins=n_bins_zone3 - 1, range=(t_quad3, t_quad4)
            )
            ...
            hist_values, bin_edges = np.histogram(
                xs[x4], bins=n_bins_zone4 - 1, range=(t_quad5, t_quad6)
            )
            ...
            if self._headless:
                Log.i(
                    f"Compare calculated stop to actual stop: {t_blip3} -> {xs[list(maxima_dict_1.keys())[-1]]}"
                )
            start_stop = [
                t_fill_start,
                t_fill_end,
            ...
            if start_at != None:
                if len(start_at) == 6:
                    if start_at[-3] != None:
                        ...
                        curr_xs_idx = next(
                            x for x, y in enumerate(xs) if y >= xs_extend[curr_xs_idx]
                        )
            ...
            for i in range(len(zone2_pois)):
                ...
                curr_xs_idx = next(
                    x for x, y in enumerate(xs_extend) if y >= xs[curr_xs_idx]
                )
                ...
                while True:
                    ...
                    if min_time < xs_extend[curr_xs_idx] < max_time:
                        if end_compare_result == 0:
                            if i == 0:
                                Log.d(
                                    f"Moved blip1 by {curr_xs_idx-start_x_idx} points when fine-tuning."
                                )
                        ...
                        if i == 0:
                            Log.d(
                                "Fine-tuning point for blip1 not found. Leaving it as-is."
                            )
                ...
                while True:
                    ...
                    if min_time < xs_extend[curr_xs_idx] < max_time:
                        if end_compare_result == 0:
                            if i == 0:
                                Log.d(
                                    f"Moved blip2 by {curr_xs_idx-start_x_idx} points when fine-tuning."
                                )
                        ...
                        if i == 0:
                            Log.d(
                                "Fine-tuning point for blip2 not found. Leaving it as-is."
                            )
                ...
                while True:
                    ...
                    if min_time < xs_extend[curr_xs_idx] < max_time:
                        if end_compare_result == 0:
                            if i == 0:
                                Log.d(
                                    f"Moved blip3 by {curr_xs_idx-start_x_idx} points when fine-tuning."
                                )
                        ...
                        if i == 0:
                            Log.d(
                                "Fine-tuning point for blip3 not found. Leaving it as-is."
                            )
            ...
            for i in range(len(start_stop)):
                try:
                    start_stop[i] = next(
                        x for x, y in enumerate(xs_extend) if y >= start_stop[i]
                    )
                except:
                    start_stop[i] = -1
            poi_vals = start_stop
                ...
                if len(start_at) == 6:
                    for i in range(len(start_at)):
                        ...
                        if poi_idx != None and poi_idx in [-1, len(xs_extend) - 1]:
                            Log.d(
                                f"Not fine-tuning POI ID{i} as it is marked as not present in dataset."
                            )
            ...
            if t_blip3 - t_fill_start < 3.0:
                Log.e(
                    TAG,
                    "Bad run due to short duration between calculated start and stop times",
                )
                is_good = False
            if min_freq < avg_start - t_0p5:
                Log.e(
                    TAG,
                    "Bad run due to 'resonance_frequency' minimum being after run start.",
                )
                is_good = False
            if max_diss < avg_start - t_0p5:
                Log.e(
                    TAG, "Bad run due to 'dissipation' minimum being after run start."
                )
                is_good = False
            if not is_good:
                poi_vals = -1
            ...
            if start_at != None:
                if len(start_at) == 6:
                    if start_at[-1] != None and start_at[-1] not in [
                        len(xs_extend - 1) - 1,
                        -1,
                    ]:
                        if xs_factor_d < 75:
                            Log.w(
                                "Ignoring models partial fill assessment as user explictly provided an existing stop point."
                            )
                            xs_factor_d = 100
            if xs_factor_d < 75:
                ...
                start_deriv = max(
                    norm_smooth_diss_1st[avg_start],
                    norm_smooth_freq_1st[avg_start],
                    norm_smooth_diff_1st[avg_start],
                    norm_smooth_diss_2nd[avg_start],
                    norm_smooth_freq_2nd[avg_start],
                    norm_smooth_diff_2nd[avg_start],
                )
                stop_deriv = max(
                    norm_smooth_diss_1st[avg_stop],
                    norm_smooth_freq_1st[avg_stop],
                    norm_smooth_diff_1st[avg_stop],
                    norm_smooth_diss_2nd[avg_stop],
                    norm_smooth_freq_2nd[avg_stop],
                    norm_smooth_diff_2nd[avg_stop],
                )
                if stop_deriv < consider_points_above_pct and not poi_vals[-1] in [
                    len(xs) - 1,
                    -1,
                ]:
                    Log.w(
                        "Partial fill run detected. Not all channels appear to be available in dataset. Please confirm points."
                    )
            ...
            if poi_vals[0] > len(xs) - 1 or xs[poi_vals[0]] != xs_extend[poi_vals[0]]:
                for i in range(len(poi_vals)):
                    poi_vals[i] = next(
                        x for x, y in enumerate(xs) if y >= xs_extend[poi_vals[i]]
                    )
            if not self._plt_init:
                self._plt_init = True
                self.fig = plt.figure()
            ...
            if not init_fill:
                self.ax3.plot(
                    xs[:idx_quad5],
                    self.normalize(super_smooth_diss_1st)[:idx_quad5],
                    color="yellow",
                    linestyle="solid",
                )
                self.ax3.plot(
                    xs[:idx_quad5],
                    self.normalize(super_smooth_diss_2nd)[:idx_quad5],
                    color="pink",
                    linestyle="solid",
                )
                self.ax3.plot(
                    xs[:idx_quad5],
                    self.normalize(super_smooth_freq_1st)[:idx_quad5],
                    color="yellow",
                    linestyle="dashed",
                )
                self.ax3.plot(
                    xs[:idx_quad5],
                    self.normalize(super_smooth_freq_2nd)[:idx_quad5],
                    color="pink",
                    linestyle="dashed",
                )
                self.ax3.plot(
                    xs[:idx_quad5],
                    self.normalize(super_smooth_diff_1st)[:idx_quad5],
                    color="yellow",
                    linestyle="dotted",
                )
                self.ax3.plot(
                    xs[:idx_quad5],
                    self.normalize(super_smooth_diff_2nd)[:idx_quad5],
                    color="pink",
                    linestyle="dotted",
                )
                self.ax3.plot(
                    xs[idx_quad5:],
                    self.normalize(np.abs(super_smooth_diss_1st))[idx_quad5:],
                    color="yellow",
                    linestyle="solid",
                )
                self.ax3.plot(
                    xs[idx_quad5:],
                    self.normalize(np.abs(super_smooth_diss_2nd))[idx_quad5:],
                    color="pink",
                    linestyle="solid",
                )
                self.ax3.plot(
                    xs[idx_quad5:],
                    self.normalize(np.abs(super_smooth_freq_1st))[idx_quad5:],
                    color="yellow",
                    linestyle="dashed",
                )
                self.ax3.plot(
                    xs[idx_quad5:],
                    self.normalize(np.abs(super_smooth_freq_2nd))[idx_quad5:],
                    color="pink",
                    linestyle="dashed",
                )
                self.ax3.plot(
                    xs[idx_quad5:],
                    self.normalize(np.abs(super_smooth_diff_1st))[idx_quad5:],
                    color="yellow",
                    linestyle="dotted",
                )
                self.ax3.plot(
                    xs[idx_quad5:],
                    self.normalize(np.abs(super_smooth_diff_2nd))[idx_quad5:],
                    color="pink",
                    linestyle="dotted",
                )
            ...
            self.ax3.axhline(
                consider_points_above_pct, color="grey", linestyle=":", alpha=0.75
            )
            if idx_max_freq != -1:
            ...
            if init_fill:
                ...
                self.ax3.plot(
                    xs_extend,
                    np.polyval(best_post_pointfit, xs_extend),
                    color="black",
                    linestyle="solid",
                    alpha=0.5,
                )
                self.ax3.plot(
                    xs_extend,
                    np.polyval(best_max_pointfit, xs_extend),
                    color="black",
                    linestyle="solid",
                    alpha=0.5,
                )
                ...
                self.ax3.annotate(
                    str(int(method)),
                    (xs_extend[debug_point1], break_diff_high),
                    ha="left",
                    va="top",
                )
                self.ax3.annotate(
                    str(best_num_fill_pts),
                    (xs_extend[best_end_idx], up3_fill[0][best_end_idx]),
                    ha="left",
                    va="bottom",
                )
                self.ax3.plot(
                    xs_extend, norm_ys_diff_soft, color="black", linestyle="dotted"
                )
                xlim_min = xs_extend[debug_point1] - 1
                xlim_max = xs_extend[best_post_idx] + 1
                ylim_min = -0.05
                ylim_max = (
                    np.amax(ys_diff_smooth_fill[debug_point1:best_post_idx]) + 0.05
                )
    ...
    def run(self):
        ...
        for i, f in enumerate(training_files):
            try:
                ...
                if "BAD" in f.upper():
                    raise ImportError()
                if f.endswith("_poi.csv"):
                    raise ImportError()
                if f.endswith("_lower.csv"):
                    raise ImportError()
                ...
                stable_streak = 0
                for sf in np.arange(
                    Constants.sf_min, Constants.sf_max, Constants.sf_step
                ):
                    calculated_pois = self.IdentifyPoints(
                        data_path, sf=sf, i=i, actual=actual_pois
                    )
