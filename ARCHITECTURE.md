...
| `tiers.py` | Data-driven `TierScheme` (GMM/quantile) | `corpus` |
...
| `inference/config.py` | `QModelV6Config` tunables | — |
| `inference/crosscheck.py` | Zoom-detector fill-count rescue/veto | — |
| `inference/controller.py` | `QModelV6YOLO` production controller | `inference.config`, `inference.crosscheck`, `decode.*`, `rendering.*` |
